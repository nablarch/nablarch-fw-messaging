 ...
 * @author Iwauo Tajima
 */
public class TestJmsMessagingProvider implements MessagingProvider {
    ...
    public MessagingContext createContext() {
        try {
            ...
            throw new MessagingException("failed to create context", e);
        }
    }
    ...
    public TestJmsMessagingProvider setConnectionFactory(ConnectionFactory factory) {
     ...
     */
    @Published(tag = "architect")
    public TestJmsMessagingProvider setDestinations(Map<String, Queue> table) {
        queueTable.clear();
     ...
     * @return このオブジェクト自体
     */
    public TestJmsMessagingProvider setPoisonQueueNamePattern(String pattern) {
     ...
     * @return このオブジェクト自体
     */
    public TestJmsMessagingProvider setDefaultPoisonQueue(String queueName) {
     ...
     * @return このオブジェクト自体。
     */
    public TestJmsMessagingProvider setRedeliveryLimit(int limit) {
    ...
    public static class Context extends MessagingContext {
        ...
        /** 各種設定 */
        private final TestJmsMessagingProvider provider;
         ...
         */
        public Context(Connection conn, TestJmsMessagingProvider provider)
        throws JMSException {
            this.conn     = conn;
         ...
         * <p/>
         * 送信電文デフォルト有効期間({@link TestJmsMessagingProvider#timeToLive})を生成したMessageProducerに設定する。
         * 
        ...
        public String sendMessage(SendingMessage message) {
            try {
                ...
                throw new MessagingException("an error occurred while sending the message.", e);
            }
        }
        ...
        receiveMessage(String queueName, String messageId, long timeout) {
            ...
            try {
                ...
                throw new MessagingException("an error occurred while receiving a message.", e);
            } finally {
                try {
        ...
        protected String getQueueNameOf(Destination queue) {
            for (Entry<String, Queue> entry : provider.queueTable.entrySet()) {
